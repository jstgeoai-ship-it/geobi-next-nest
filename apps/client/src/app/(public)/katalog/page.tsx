  ...
  return (
    <div className="pt-24 px-6 pb-16 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-slate-400 mb-10">Pilih dashboard sistem geospasial</p>

        <div className="grid md:grid-cols-3 gap-6">
          {DASHBOARDS.map((d) => (
            <Link key={d.href} href={d.href} className="bg-slate-900 p-6 rounded-xl hover:bg-slate-800 transition-colors">
              <h2 className="text-white text-xl font-bold">{d.title}</h2>
              <p className="text-slate-400 mt-2">{d.desc}</p>
            </Link>
          ))}
        </div>
      </div>
