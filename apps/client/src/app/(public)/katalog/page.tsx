  ...
  return (
    <div className="pt-24 px-6 pb-16 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-[var(--pub-muted-2)] light:text-[var(--pub-muted-3)] mb-10">Pilih dashboard sistem geospasial</p>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {DASHBOARDS.map((d) => (
            <Link key={d.href} href={d.href} className="bg-slate-900 light:bg-slate-50 light:border light:border-gray-200 p-6 rounded-xl hover:bg-slate-800 light:hover:bg-gray-50 transition-colors">
              <h2 className="text-[var(--pub-text)] light:text-[var(--pub-text)] text-xl font-bold">{d.title}</h2>
              <p className="text-[var(--pub-muted-2)] light:text-[var(--pub-muted-3)] mt-2">{d.desc}</p>
            </Link>
          ))}
        </div>
      </div>
